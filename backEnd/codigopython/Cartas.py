...
class Cartas:
    ...
    def printerMaoJogador(maoPlayer):
        ...
        for i, carta in enumerate(maoPlayer):
            valor_formatado = nomes_valores.get(
                carta.ordemDeValor, str(carta.ordemDeValor)
            )
            naipe_formatado = nomes_naipes[carta.naipe - 1]
            match carta.naipe:
                case 1:
                    print(
                        f"[Carta {i + 1}]\033[32m {valor_formatado:<6} | de {naipe_formatado}\033[0m"
                    )
                case 2:
                    print(
                        f"[Carta {i + 1}]\033[33m {valor_formatado:<6} | de {naipe_formatado}\033[0m"
                    )
                case 3:
                    print(
                        f"[Carta {i + 1}]\033[36m {valor_formatado:<6} | de {naipe_formatado}\033[0m"
                    )
                case 4:
                    print(
                        f"[Carta {i + 1}]\033[31m {valor_formatado:<6} | de {naipe_formatado}\033[0m"
                    )
        print()
